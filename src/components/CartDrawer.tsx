              ...
              <div key={item.productId} className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[var(--color-jade-pink)] font-medium mt-1">${item.price.toFixed(2)}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded">
                      <button 
                        ...
                        className="p-1 px-2 hover:bg-[var(--jade-bg)] text-gray-500"
                        disabled={item.qty <= 1}
                      >
                        <Minus size={14} />
                      </button>
                      <span className="text-sm w-8 text-center text-[var(--jade-text)]">{item.qty}</span>
                      <button 
                        onClick={() => updateQty(item.productId, item.qty + 1)}
                        className="p-1 px-2 hover:bg-[var(--jade-bg)] text-gray-500"
                      >
        ...
        {cart.length > 0 && (
          <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-[var(--jade-bg)]/50">
            <div className="flex justify-between items-center mb-4">
              <span className="font-medium text-gray-600 uppercase tracking-widest text-sm">Subtotal</span>
              <span className="font-serif text-2xl font-medium">${cartTotal.toFixed(2)}</span>
            </div>
            <p className="text-xs text-gray-500 mb-6 font-mono">Shipping and taxes calculated at checkout.</p>
