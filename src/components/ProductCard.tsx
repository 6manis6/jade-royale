...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/products/${product._id}`} className="group relative block bg-[var(--jade-card)] hover:shadow-xl transition-all duration-300 border border-transparent dark:border-gray-800">
      {/* Product Image Area */}
      <div className="relative h-80 bg-[var(--jade-bg)] dark:bg-gray-900/50 overflow-hidden flex items-center justify-center p-6">
        {product.badge && (
          <span className={`absolute top-4 left-4 text-xs font-bold text-white px-2 py-1 rounded-sm z-10 ${product.badge === 'SALE' ? 'bg-red-500' : product.badge === 'NEW' ? 'bg-gray-900' : 'bg-[var(--color-jade-pink)]'}`}>
            {product.badge}
          </span>
        )}
      ...
      <div className="text-center p-6">
        ...
        <div className="flex items-center justify-center gap-2">
          {product.originalPrice && (
            <span className="text-black dark:text-gray-500 line-through text-sm font-medium opacity-60">${product.originalPrice.toFixed(2)}</span>
          )}
          <span className="text-[var(--color-jade-pink)] font-semibold">${product.price.toFixed(2)}</span>
        </div>
      </div>
    </Link>
  );
